...
module.exports = {
  ...
  async removeSingleRatingReviewController(req, res) {
    try {
      ...
      const newError = createErrorMessage();
      newError.data = error;
      newError.message = "Delete Rating Controller Error";
      newError.status = statusCode.internalServerError;
      newError.error = true;
      return res.status(newError.status).json(newError);
    }
  ...
  async activateSingleRatingReviewController(req, res) {
    try {
      let response = await ratingReviewServices.activateSingleRatingReviewService(req.params);
      return res.status(response.status).send(response);
    } catch (error) {
      console.error(error);
      const newError = createErrorMessage();
      newError.data = error;
      newError.message = "Activate Rating Controller Error";
      newError.status = statusCode.internalServerError;
      newError.error = true;
      return res.status(newError.status).json(newError);
    }
